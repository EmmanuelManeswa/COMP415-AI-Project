...
void Tree::create_bombs()
{
    ...
    std::uniform_int_distribution<int> distribution(0, ROWS-1); // Uniform discrete distribution. produces random integers in a range [0,9].
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 2; j++)
        {
...
{
    if (bombs[0][0] == bombs[1][0] && bombs[0][1] == bombs[1][1])
        return true;
    if (bombs[0][0] == bombs[2][0] && bombs[0][1] == bombs[2][1])
        return true;
    if (bombs[1][0] == bombs[2][0] && bombs[1][1] == bombs[2][1])
        return true;
    return false;
}
...
bool Tree::if_top(node_ptr move, int row_no, int col_no) const
{
    if (row_no < 0)
        return false;
    if (move->board[row_no][col_no] == square::visited_square)
        return false;
...
void Tree::dfs_algorithm(node_ptr branch)
{
    ...
    if (!is_goal(branch))
    {
        ...
        if (branch->bottom != nullptr)
            stack.push(branch->bottom);
        if (branch->right != nullptr)
            stack.push(branch->right);
        if (branch->left != nullptr)
            stack.push(branch->left);
        if (branch->top != nullptr)
            stack.push(branch->top);
...
{
    if (problem[0] >= COLUMNS || problem[1] >= ROWS || problem[0] < 0 || problem[1] < 0 || problem[2] >= COLUMNS || problem[3] >= ROWS || problem[2] < 0 || problem[3] < 0)
        return false;
    return true;
}
